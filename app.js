...
function calculateBRAA() {
    ...
    if (ownLat === null || ownLon === null || isNaN(radial) || isNaN(dist)) {
        el.resBearing.textContent = "---°"; el.resRange.textContent = "---"; return;
    }

    const trueRadial = (radial + magVar + 360) % 360;
    const targetPos = getDestPoint(bullLat, bullLon, trueRadial, dist);
    ...
    el.resBearing.textContent = Math.round(magBearingToTarget).toString().padStart(3, '0') + '°';
    el.resRange.textContent = Math.round(rangeToTarget);
...
function calculateBullCoord() {
    ...
    if (isNaN(radial) || isNaN(dist)) {
        el.bullResLat.textContent = "S 00° 00.00'"; el.bullResLon.textContent = "W 000° 00.00'"; el.resMGRS.textContent = "---"; return;
    }

    const trueRadial = (radial + magVar + 360) % 360;
    const targetPos = getDestPoint(bullLat, bullLon, trueRadial, dist);
    el.bullResLat.textContent = toDDM(targetPos.lat, true);
    el.bullResLon.textContent = toDDM(targetPos.lon, false);
    
    try {
        el.resMGRS.textContent = mgrs.forward([targetPos.lon, targetPos.lat], 4);
    } catch(e) { el.resMGRS.textContent = "ERR"; }
}
...
function renderThreatConfig() {
    el.threatsConfigList.innerHTML = state.threats.map((t, index) => `
        <div class="threat-config-item">
            <span class="threat-badge">${t.type}</span>
            <span class="threat-code">${t.code}</span>
            <span class="threat-val">${t.range} NM</span>
            <button class="btn-delete-threat" onclick="deleteThreat(${index})">×</button>
        </div>
    `).join('');
}

function deleteThreat(index) {
    state.threats.splice(index, 1); updateThreatDropdowns();
}
...
function addPlot() {
    ...
    const threatCode = el.threatSelect.value;
    const threat = state.threats.find(t => t.code === threatCode);
    const targetId = el.targetId.value || "1";
    
    state.plots.push({
        ...state.targetPos,
        targetId: targetId,
        threatCode: threatCode || null,
                ...
                <div class="history-group-content">
                    <table class="tactical-table">
                        ...
                        <tbody>
                            ${items.map(plot => `
                                <tr>
                                    <td><input type="number" value="${plot.radial}" onchange="updatePlot(${plot.originalIndex}, 'radial', this.value)"></td>
...
window.updatePlot = (index, field, value) => {
    ...
    if (field === 'threatCode') {
        ...
        plot.threatCode = value || null; plot.threatRange = threat ? threat.range : null; plot.threatType = threat ? threat.type : null;
    ...
    plot.lat = pos.lat; plot.lon = pos.lon;
    drawTacticalDisplay(); renderHistory();
};
...
function drawTacticalDisplay() {
    ...
    state.plots.forEach(plot => {
        ...
        if (isLatest) {
            ...
            const label = plot.threatType === 'A/A' ? 'PUMP CRIT' : '';
            ctx.fillText(`${plot.targetId} ${plot.threatCode || ''}`, x + 10, y + 5);
            if (label) { ctx.fillStyle = 'rgba(255, 0, 0, 0.9)'; ctx.fillText(label, x + 10, y + 16); }
            if (plot.threatRange) {
...
function updatePosition(pos) {
    ...
    el.ownLat.value = latitude.toFixed(6); el.ownLon.value = longitude.toFixed(6);
    ...
    state.ownPos.lat = latitude; state.ownPos.lon = longitude; state.ownPos.heading = heading || 0;
    ...
    el.gpsStatus.classList.remove('offline'); el.gpsStatus.classList.add('online');
    calculateBRAA();
}
...
function handleGPSError(err) {
    ...
    el.gpsStatus.textContent = msg; el.gpsStatus.classList.add('offline');
    if (err.code === 3) navigator.geolocation.getCurrentPosition(updatePosition, null, { enableHighAccuracy: false, timeout: 5000 });
}

function initGPS() {
    if (!navigator.geolocation) return;
    el.gpsStatus.textContent = "GPS: BUSCANDO..."; el.gpsStatus.classList.add('offline');
    ...
    navigator.geolocation.getCurrentPosition(updatePosition, (err) => {
        if (err.code === 3) navigator.geolocation.getCurrentPosition(updatePosition, handleGPSError, { enableHighAccuracy: false, timeout: 10000 });
...
el.navBtns.forEach(btn => btn.addEventListener('click', () => {
    ...
    if (btn.getAttribute('data-page') === 'calc-page') setTimeout(drawTacticalDisplay, 100);
}));
