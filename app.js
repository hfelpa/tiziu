...
function parseGGMM_MM(str, isLat) {
    ...
    let sign = -1; // Default to South / West in tactical context if no N/E is given
    if (clean.includes('N') || clean.includes('E') || clean.startsWith('+')) {
        sign = 1;
    } else if (clean.includes('S') || clean.includes('W') || clean.startsWith('-')) {
        sign = -1;
    }

    // Clean out symbols except digits, dot and spaces
    let stripped = clean.replace(/[NSWE+°'"-]/g, ' ').trim();
    
    // Check if space-separated: e.g. "15 30.00" or "049 30.50"
    const parts = stripped.split(/\s+/).filter(Boolean);
    if (parts.length >= 2) {
        const deg = parseFloat(parts[0]);
        const min = parseFloat(parts[1]);
        if (!isNaN(deg) && !isNaN(min) && min < 60) {
            return sign * (Math.abs(deg) + min / 60);
        }
    }

    // Compact format: e.g. "1530.00" or "04930.00" or "1530"
    let compacted = stripped.replace(/\s/g, '');
    const dotIndex = compacted.indexOf('.');
    let degPart = "";
    let minPart = "";
    if (dotIndex !== -1) {
        const wholePart = compacted.substring(0, dotIndex);
        const decPart = compacted.substring(dotIndex + 1);
        if (wholePart.length < 2) return NaN;
        minPart = wholePart.slice(-2) + '.' + decPart;
        degPart = wholePart.slice(0, -2);
    } else {
        if (compacted.length < 2) return NaN;
        minPart = compacted.slice(-2);
        degPart = compacted.slice(0, -2);
    }
...
function updateBullseyesTable() {
    ...
    state.bullseyes.forEach((b, index) => {
        ...
        row.innerHTML = `
            ...
            <td class="bullseye-cell-val">${toDDM(b.lat, true)}</td>
            <td class="bullseye-cell-val">${toDDM(b.lon, false)}</td>
